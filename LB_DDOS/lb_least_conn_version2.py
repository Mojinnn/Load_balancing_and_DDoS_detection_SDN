...
class LeastConnLB(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Counter theo src_ip (single-source)
        self.ip_counter      = collections.defaultdict(int)
        self.ip_time_window  = {}
        self.IP_THRESHOLD    = 50

        # Counter theo dst_port (rand-source)
        self.port_counter     = collections.defaultdict(int)
        self.port_time_window = {}
        self.PORT_THRESHOLD   = 200
        self.PORT_WINDOW      = 10
        self.REQ_WINDOW       = 10  # dùng chung cho cả 2
    # ─────────────────────────────────────────────────────
    #  Helpers
    # ─────────────────────────────────────────────────────
    ...
    def flow_removed_handler(self, ev):
        ...
        if cookie == COOKIE_DROP:
            ...
            if dst_port:
                # Unblock port (rand-source)
                self.blocked_ports.discard(dst_port)
                self.port_counter[dst_port] = 0
                self.port_time_window.pop(dst_port, None)
                self.logger.info(f"[DDOS] Unblocked port {dst_port} — resuming LB")

            if src_ip:
                # Unblock IP (single-source)
                self.blocked_ips.discard(src_ip)
                self.ip_counter[src_ip] = 0
                self.ip_time_window.pop(src_ip, None)
                self._cleanup_sessions_by_ip(src_ip)
                self.logger.info(f"[DDOS] Unblocked IP {src_ip}")

            self._save_stats()
            return

        # COOKIE_LB — dọn session như cũ
        src_ip = match.get('ipv4_src')
        if not src_ip:
            return
        if cookie == COOKIE_LB:
    ...
    def _ddos_hook(self, dp, src_ip, src_port, dst_ip,
        ...
        # Track 1: theo src_ip
        # ════════════════════════════
        if src_ip not in self.ip_time_window:
            self.ip_time_window[src_ip] = now
            self.ip_counter[src_ip]     = 0
        if now - self.ip_time_window[src_ip] > self.REQ_WINDOW:
            self.ip_time_window[src_ip] = now
            self.ip_counter[src_ip]     = 0
        self.ip_counter[src_ip] += 1

        # ════════════════════════════
        # Track 2: theo dst_port
        # ════════════════════════════
        if dst_port not in self.port_time_window:
            self.port_time_window[dst_port] = now
            self.port_counter[dst_port]     = 0
        if now - self.port_time_window[dst_port] > self.PORT_WINDOW:
            self.port_time_window[dst_port] = now
            self.port_counter[dst_port]     = 0
        ...
        # 1 IP vượt ngưỡng → block IP
        # ════════════════════════════
        if self.ip_counter[src_ip] >= self.IP_THRESHOLD:
            ...
            if is_attack:
                self.logger.warning(
                    f"[DDOS] Single-source: {src_ip} → block IP")
                self.blocked_ips.add(src_ip)
                self._install_drop_ip(dp, src_ip)
                return True

        # ════════════════════════════
        # Case 2: rand-source
        # Tổng port vượt ngưỡng → block port
        # ════════════════════════════
        if self.port_counter[dst_port] >= self.PORT_THRESHOLD:
            is_attack = self.detector.is_attack(
                src_ip, src_port, dst_ip,
                dst_port, ip4_proto, tcp_flags
            )
            if is_attack:
                self.logger.warning(
                    f"[DDOS] Rand-source: port {dst_port} flooded → block port")
                self.blocked_ports.add(dst_port)
                self._install_drop(dp, dst_port)
                return True
    ...
    def _install_drop(self, dp, dst_port: int):
        parser = dp.ofproto_parser
        ofp    = dp.ofproto
        ...
        mod = parser.OFPFlowMod(
            ...
            instructions=inst,
            idle_timeout=0,
            hard_timeout=60,        # block 60s rồi tự mở
            flags=ofp.OFPFF_SEND_FLOW_REM
        )
        dp.send_msg(mod)
        self.logger.warning(f"[DDOS] DROP installed for VIP:{dst_port} (60s)")
    def _install_drop_ip(self, dp, src_ip: str):
        parser = dp.ofproto_parser
        ofp    = dp.ofproto
        match  = parser.OFPMatch(
            eth_type=0x0800,
        ...
        dp.send_msg(mod)
        self.logger.warning(f"[DDOS] DROP IP {src_ip} (60s)")
